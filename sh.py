...
def verify_user_password(cursor, username, plain_password):
    cursor.execute(
        """
        SELECT username, mobile, password, password_hash, is_active, role, failed_attempts
        FROM dbo.users
        WHERE username=?
        """,
        (username,)
    ...
    if user[4] is not None and int(user[4]) == 0:
        return "inactive"

    if user[6] is not None and int(user[6]) >= 5:
        return "locked"

    password_hash = user[3] or ""
    old_password = user[2] or ""
    valid = False
...
def login():
    ...
    try:
        ...
        entered_mobile = request.form.get('mobile', '').strip()
    ...
    if user:
        registered_mobile = user[1] or ""
        login_mobile = normalize_mobile(entered_mobile)
        saved_mobile = normalize_mobile(registered_mobile)

        if not saved_mobile:
            flash("Registered mobile number not found. Please contact admin.", "error")
            return redirect('/login')

        if login_mobile and login_mobile != saved_mobile:
            log_activity("login_failed", "Mobile number mismatch")
            flash("Mobile number does not match registered mobile.", "error")
            return redirect('/login')

        session['temp_user'] = user[0]
        session['temp_role'] = user[5] or "user"
        session['otp_mobile'] = saved_mobile
        session['remember_device'] = remember_device
        otp = str(random.randint(100000, 999999))
        session['otp'] = otp
        session['otp_expires_at'] = (datetime.now() + timedelta(minutes=5)).isoformat()
        sent, sms_message = send_otp_sms(saved_mobile, otp)
        if not sent:
            print(f"OTP for {saved_mobile}: {otp}")
            sent = True
            sms_message = "OTP SMS failed, so OTP printed in console"
        session['otp_message'] = sms_message
        session['otp_mobile_masked'] = mask_mobile(saved_mobile)
...
def otp_page():
    ...
    return render_template(
        'otp.html',
        otp_mobile=session.get('otp_mobile_masked', ''),
...
def resend_otp():
    if not session.get('temp_user') or not session.get('otp_mobile'):
        return redirect('/login')

    otp = str(random.randint(100000, 999999))
    session['otp'] = otp
    session['otp_expires_at'] = (datetime.now() + timedelta(minutes=5)).isoformat()
    sent, sms_message = send_otp_sms(session.get('otp_mobile'), otp)
    if not sent:
        print(f"OTP for {session.get('otp_mobile')}: {otp}")
        sent = True
        sms_message = "OTP SMS failed, so OTP printed in console"
    session['otp_message'] = sms_message
    session['otp_mobile_masked'] = mask_mobile(session.get('otp_mobile'))
...
def settings():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT profile_pic FROM dbo.users WHERE username=?",
            (session.get("username"),)
        )
        row = cursor.fetchone()
        if row and row[0]:
